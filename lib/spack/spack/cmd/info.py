...
class VariantFormatter(object):
    def __init__(self, variants):
        ...
        # Minimize column widths according to max widths, but allow the
        # description column to extend as wide as the terminal.
        _, cols = tty.terminal_size()
        max_name = min(self.column_widths[0], 30)
        max_vals = min(self.column_widths[1], 20)
        max_description = min(
            self.column_widths[2],
            cols - max_name - max_vals - 14  # 14 is margins + column spacing
        )
